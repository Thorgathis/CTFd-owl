...
- Пример мультизадачи (один контейнер, несколько связанных тасок и флагов) приведён в `CTFd/plugins/ctfd-owl/source/tasks/dynamic-multitask`.

Во всех случаях контейнер получает `FLAG` автоматически (смотрите `FLAG=${FLAG}` в compose-файлах примеров).

#### Динамическая мультизадача (`dynamic_check_docker_multi`)

Мультизадача запускает **один общий контейнер, обслуживающий несколько связанных тасок**. Каждая таска получает свой пер-инстансный динамический флаг, который пробрасывается в контейнер как `FLAG1`, `FLAG2`, ... `FLAGN` (`FLAG` равен `FLAG1` для обратной совместимости). Контейнер отображается запущенным на **всех** тасках группы, и каждая таска принимает только свой флаг.

- При создании выберите тип **Dynamic multitask**, укажите **Dirname** (в нём лежит общий docker-compose) и **Number of flags / tasks** (N). Созданная задача — это родитель группы (таска №1); остальные N−1 тасок создаются автоматически как дочерние задачи и наследуют статус видимости родителя (и следуют за ним при последующем изменении) — отредактируйте их при необходимости.
- Compose должен пробрасывать переменные окружения `FLAG1..FLAGN` (см. пример `dynamic-multitask`). Таска `k` решается значением `FLAG<k>`.
- Запуск, продление и удаление инстанса из любой таски группы затрагивают всю группу; группа считается одним инстансом в лимитах на пользователя/команду.
- Позже можно добавлять, удалять или перенумеровывать таски со страницы редактирования задачи (`Flag index` / `Parent challenge id`), например чтобы сделать 2 или 4 флага вместо 3.
